...
function ContentReviewDialog({ detail, preview, request, busy, t, onCancel, onConfirm }: { detail: FileLibraryDetail; preview: ContentPreview; request: ContentPreviewRequest; busy: boolean; t: Translator; onCancel: () => void; onConfirm: () => void }) {
  ...
  return <ModalPortal initialFocusRef={closeRef} onEscape={onCancel}>
    <div className="fixed inset-0 z-40 grid place-items-center bg-[var(--zc-overlay)] p-5" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onCancel(); }}>
      <section className={cn(overlaySurface, "grid w-full max-w-2xl gap-4 p-5")} role="dialog" aria-modal="true" aria-labelledby="content-review-title">
        ...
        <div className="flex justify-end gap-2"><button type="button" className={buttonSecondary} disabled={busy} onClick={onCancel}>{t("contentCancel")}</button><button type="button" className={buttonPrimary} disabled={busy || preview.exactState === "deferred" || preview.supportedCount === 0} onClick={onConfirm}>{t("contentConfirmStart")}</button></div>
      </section>
    </div>
  </ModalPortal>;
}
