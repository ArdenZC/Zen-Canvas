...
import { buttonGhost, cn, contentPanel, emptyState, focusVisibleState, glassButtonPrimary, selectedSurface } from "../../utils/tw";
        ...
        <section className={cn(panelSurface, "grid gap-4 overflow-visible p-4 lg:grid-cols-[minmax(260px,0.8fr)_minmax(0,1.2fr)]")}>
          <div className={cn("grid min-w-0 content-start gap-3", isNarrow && narrowPane === "details" && "hidden")}>
            ...
            <div className="flex max-w-full flex-wrap items-center gap-1" role="group" aria-label={t("historyBatches")}>
              {primaryFilterButtons.map(({ value, key }) => <button key={value} type="button" aria-pressed={filter === value} className={cn(buttonGhost, "shrink-0", filter === value && selectedSurface)} onClick={() => changeFilter(value)}>{t(key)}</button>)}
              <div ref={moreFiltersRef} className="relative">
                <button ref={moreFiltersTriggerRef} type="button" className={cn(buttonGhost, "shrink-0", !primaryFilterButtons.some((item) => item.value === filter) && selectedSurface)} aria-haspopup="dialog" aria-expanded={moreFiltersOpen} aria-controls="history-more-filters" onClick={() => setMoreFiltersOpen((current) => !current)}>
                ...
                {moreFiltersOpen ? <div id="history-more-filters" role="dialog" aria-label={t("historyMoreFilters")} className="absolute left-0 top-full z-20 mt-2 grid min-w-52 gap-1 rounded-[var(--zc-radius-panel)] border border-[var(--zc-border)] bg-[var(--zc-surface-floating)] p-2 shadow-[var(--zc-shadow-floating)]">
                  {moreFilterButtons.map(({ value, key }) => <button key={value} type="button" aria-pressed={filter === value} className={cn(buttonGhost, "justify-start", filter === value && selectedSurface)} onClick={() => changeMoreFilter(value)}>{t(key)}</button>)}
                </div> : null}
              </div>
            </div>
