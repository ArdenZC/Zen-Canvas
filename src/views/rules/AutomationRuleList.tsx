...
export function AutomationRuleList({ rules, activeId, busyRuleIds, toggleErrorIds, listRef, onSelect, onFocus, onToggle, lastRunLabel, t }: {
  rules: readonly Rule[];
  activeId: string;
  ...
  return (
    <div className="grid min-w-0 gap-1 overflow-x-auto" data-automation-rule-table="true">
      <div className="hidden min-w-[34rem] grid-cols-[minmax(0,1.35fr)_minmax(7rem,.7fr)_minmax(8rem,.8fr)_auto] gap-3 px-3 text-[11px] font-semibold text-[var(--zc-text-tertiary)] sm:grid" aria-hidden="true">
        <span>{t("ruleName")}</span>
        <span>{t("automationTrigger")}</span>
        <span>{t("automationRuleLastResult")}</span>
        <span>{t("automationStatus")}</span>
      </div>
      <ul ref={listRef} role="list" aria-label={t("automationRules")} className="grid min-w-[34rem] gap-1 outline-none">
      {rules.map((rule, index) => {
        const active = rule.id === activeId;
        const busy = busyRuleIds.has(rule.id);
        return (
          <li key={rule.id} className={cn("grid grid-cols-[minmax(0,1.35fr)_minmax(7rem,.7fr)_minmax(8rem,.8fr)_auto] items-center gap-3 rounded-[var(--zc-radius-field)] border px-3 py-3 transition-colors", active ? "border-[var(--zc-primary)] bg-[var(--zc-surface-selected)]" : "border-transparent hover:border-[var(--zc-border)] hover:bg-[var(--zc-surface-hover)]")}>
            <button
            ...
            <span className="hidden truncate text-xs text-[var(--zc-text-secondary)] sm:block" title={lastRunLabel ?? t("automationNoPersistedRun")}>{lastRunLabel ?? t("automationNoPersistedRun")}</span>
