...
| TD-014 | macOS Safe Trash | open | Schema-34 cleanup identity encoding required a compatibility field for macOS source/Claim physical identity; Schema-35 implementation and reviewer remediation remain under review | Schema 35 adds the dedicated source-volume field, normalizes only provable historical source/Trash/Claim components, preserves ambiguous rows for manual review, retires the runtime tagged adapter, and passes all required exact-head migration, rollback, recovery and applicable native hardening coverage |
