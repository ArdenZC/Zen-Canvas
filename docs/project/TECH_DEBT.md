...
| TD-012 | Build assets | open | W5-02 now provides exact supported-platform package evidence, so packaging itself no longer blocks evaluation; legacy/one-off brand or installer assets may still be obsolete, but deletion could affect real consumers | Exact supported-platform packaging remains green **and** repository search/caller-equivalence proves each candidate asset has no required consumer before deletion |
...
| TD-014 | macOS Safe Trash | closed | Schema-34 cleanup identity encoding required a compatibility field for macOS source/Claim physical identity; Schema 35 now stores explicit source volume with raw source/Trash/Claim file IDs and keeps ambiguous historical evidence fail closed | Closed by PR #176 at `master@d7c96c1481caf5105ce82702ca95c2998d83b6cf`: the separately authorized migration normalized only provable historical identity, preserved untagged/mixed/conflicting rows without trust promotion, retired the runtime tagged adapter, preserved Restore/non-macOS semantics, and passed exact-head migration, rollback, recovery, macOS native/race and Windows coverage |
...
## W5 package-evidence reclassification

W5-02 produced fresh exact-tree Windows NSIS and Apple-Silicon DMG package evidence for the accepted release-hardening tree. That satisfies the specific packaging prerequisite that kept TD-012 `blocked` during W5-01.

TD-012 is therefore now `open`, not `blocked`. It is **not closed** and no asset is authorized for deletion merely because packaging passed. The second half of its exit condition — exact consumer/equivalence proof for each deletion candidate — still applies. TD-012 does not preempt the active W5 release sequence.
