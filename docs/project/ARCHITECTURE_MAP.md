...
- Provider coordination is operation-aware and treats accessor-supplied URLs as authoritative. Safe Trash uses a source/actual-target pair while Permanent Delete uses a single deleting source. Generic File Provider paths remain routing hints until the public item/domain identity callback and a runtime-applicable `managerForDomain:` manager both succeed; local APFS, portable, network and provider strategies expose separate backend probes rather than labels backed by one APFS-only claim path.
- Source retirement is an explicit capability decision (`ExclusiveClaim`, `ProviderCoordinated` or `PortableNamespaceRetirement`). Unknown/read-only/disconnect-unverified volumes fail closed and target-first cleanup remains recoverable through the existing journal and History actions.
