...
- Provider coordination is operation-aware and treats accessor-supplied URLs as authoritative. Safe Trash uses a source/actual-target pair while Permanent Delete uses a single deleting source. Generic File Provider paths are routing hints until a native item/domain identity bridge exists; local APFS, portable, network and provider strategies expose separate backend probes rather than labels backed by one APFS-only claim path.
