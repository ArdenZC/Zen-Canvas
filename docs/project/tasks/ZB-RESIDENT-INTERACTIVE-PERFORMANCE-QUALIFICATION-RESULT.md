...
For the bounded traversal/QoS repair, production commit `78d62762` contains the Rust repair, deterministic tests, and qualification harness changes. At this committed source, `cargo fmt --manifest-path src-tauri/Cargo.toml --check`, `node --check scripts/qualifyInteractiveBrowser.mjs`, Python AST syntax checks for both macOS scripts, `git diff --check`, narrow Clippy (`desktop-runtime performance-test-tauri --lib --tests -- -D warnings`) and the `desktop-runtime native-qa` binary check passed. The local scanner test executable built, but Windows could not start it (`0xc0000139 / STATUS_ENTRYPOINT_NOT_FOUND`); no Rust test ran. The focused managed-scan performance executable had the same loader failure in debug and release earlier in this continuation. These are **not test passes**; hosted qualification and CI must provide executable evidence.
...
The bounded repair checks also left task-owned generated SQLite fixtures at `F:\_codex_evidence\resident-interactive-qualification-20260926\local-temp\resident-repair-rust-tests`. The exact-path recursive cleanup was rejected twice by the automatic policy reviewer as `blocked by policy` before execution; the directory is still present with two test database files. Local task hygiene remains pending for this path as well.
...
The candidate adds deterministic coarse startup checkpoints only under `native-qa`, including process entry, Tauri setup, database readiness, runtime owners, tray, autostart, hotkey, watcher, macOS lifecycle and setup completion. Checkpoint values contain no path or user data. The `a40ec268` LLDB artifact (`10914651053`) incorrectly said `DIAGNOSTIC UNAVAILABLE` in JSON, but its retained raw trace contains `stop reason = hit Objective-C exception` and a backtrace from `objc_exception_throw` through miniz_oxide/flate2/png, `tray_icon::platform_impl::platform::icon::PlatformIcon::to_png`, `set_icon_for_ns_status_item_button`, `TrayIconBuilder::build`, and Zen `setup_tray`. That run used macOS candidate SHA-256 `c5e8b271b16e153742d43a345f4b7fe63e9f16e0b5699ad13bbe6713fb1ae01e`; the locked dependency versions were tray-icon `0.24.1`, Tauri `2.11.3`, tao `0.35.3`, wry `0.55.1`, objc2 `0.6.4` and objc2-exception-helper `0.1.1`. The `28ffd3db` parser now accepts the Objective-C stop reason, but its exact-head 45-second LLDB run (resident artifact `10914808113`, same candidate SHA-256) timed out before an exception/backtrace was captured and remains classified `DIAGNOSTIC UNAVAILABLE`; the debugger was stopped and the task was removed. Thus the prior raw stack is useful diagnostic evidence from the byte-identical app, not a captured exact-head trace or proof of the root cause. The exact-head app again aborted with `-6` after `core_runtime_owners_ready` and before `tray_ready`; evidence places the failure in the tray setup path, but does not decide whether the input image, tray-icon implementation or Apple runtime is causal. Upstream-versus-Zen attribution remains unresolved.

Browser-only qualification remains production-code-free and is configured for five independent runs on each OS at both existing viewports. On exact source `28ffd3db`, macOS and Windows each completed five runs, 90 Preview observations and 10 Browse observations with zero target misses and task cleanup true (artifacts `10914543665` and `10914747396`). macOS Preview shell/useful p95 ranges were 12.4–93.8 / 40.1–283.4 ms; Browse feedback/useful p95 ranges were 8.4–28.9 / 38.1–92.4 ms. Windows Preview shell/useful p95 ranges were 22.6–86.9 / 54.0–128.7 ms; Browse feedback/useful p95 ranges were 12.6–39.7 / 54.1–99.0 ms. Both artifacts classify **BROWSER TARGETS MET / NATIVE UI UNVERIFIED**. The four historical Preview misses were not reproduced in either final-code five-run mock series; retain them for owner disposition as potentially non-reproducible on this code head, and do not self-clear them. Browser timings do not qualify native UI latency.

The post-repair Windows gate remains one full Workspace Foundation managed-scan observation plus three independent observations. On exact `28ffd3db`, the full suite measured idle/pressure p95 **167/448 µs (2.683x, TARGET MISSED; 149 ms foreground wait)**. The three independent observations were **168/461 µs (2.744x, miss; 1695 ms wait)**, **172/1184 µs (6.884x, miss; 667 ms wait)** and **144/402 µs (2.792x, miss; 277 ms wait)**. All four structural rows were HARD PASS; each independent row recorded four pressure slots, five real scans, background progress after release, cancellation/lease release and settled runtime/scheduler. All four existing suite commands and all three observation commands exited 0, but the unchanged 2x target remains missed and the qualification is **PERFORMANCE REVIEW REQUIRED**.

The required test-only 1–4 effective-slot causal matrix completed on exact `28ffd3db` (run `36267998830`, artifact `10915195073`): on one Windows Server 2025 10.0.26100 x64 runner with four logical CPUs and the shared 100,000-entry fixture, slot 1 was 199→239 µs (1.201x, 102 ms foreground wait), slot 2 234→389 µs (1.662x, 113 ms), slot 3 185→488 µs (2.638x, 120 ms, TARGET MISSED), and slot 4 177→328 µs (1.853x, 131 ms). All four structure rows were HARD PASS; admissions matched the requested slots; cancellation released leases, workers joined, and runtime/scheduler settled. No durable scan progress marker changed during the short pressure-sampling window. Ratios did not rise monotonically with slot count, so this experiment does not support changing production resource weighting or CPU-governor behavior; no governor/frequency telemetry was collected.

The exact Windows resident app and separately qualified Global Index service each have 31 samples and classify **HARD PASS / OBSERVATIONAL MEMORY** on the same candidate SHA-256 `5B6F1A0182873EAA77CAAC4A8EC33BC0998A3EE697FDD13BEB9034FCAC91441A` (artifact `10915117293`). App WS/private committed/handles were 21,057,536→21,012,480 bytes / 6,135,808→6,074,368 bytes / 215→215; service values were 11,386,880→11,337,728 / 1,830,912→1,740,800 / 153→153. Both had zero WebView children and used the same image; the service coordinator cycle/wait deltas were zero in the settled window. Cleanup stopped/deleted the disposable service and removed the candidate process, fixture, qualification VHD and profile.

On hosted macOS 26.6.2 arm64, all preparation, existing backend suites and three managed-scan observations exited 0. Independent idle/pressure p95 values were 54/92 µs (1.704x, 176 ms wait), 67/99 µs (1.478x, 249 ms) and 60/84 µs (1.400x, 186 ms); all three were TARGET MET and structural HARD PASS with background progress after release. The real app resident test remains **BLOCKED**: candidate SHA-256 `c5e8b271b16e153742d43a345f4b7fe63e9f16e0b5699ad13bbe6713fb1ae01e` aborted with exit `-6`, produced zero samples, and stopped at `core_runtime_owners_ready`; `tray_ready` was not reached. Cleanup stopped the process and removed the task profile. The latest 45-second LLDB attempt timed out without a captured backtrace (resident artifact `10914808113`). The overall hosted qualification run `36267998830` therefore preserves **PERFORMANCE REVIEW REQUIRED / BLOCKED**. PR #269 remains Draft, no merge or AI Semantic Authority work is performed, and this record does not claim owner acceptance.
