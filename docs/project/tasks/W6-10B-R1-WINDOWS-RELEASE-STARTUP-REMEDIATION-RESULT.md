...
Owner disposition: **retain RC1 and do not create RC2 at this time**. There is no prior public Zen Canvas release/tag whose persisted state must be supported as a public upgrade boundary. The retained pre-release/dev profile incompatibility is therefore accepted as a non-blocking first-release residual, but only conditionally: a separate Windows requalification must prove that RC1 can (1) start from a true clean profile, (2) close and relaunch using state generated by RC1 itself, and (3) uninstall/reinstall while retaining that RC1-generated state without reproducing the blank-window failure. The full Windows release matrix must then complete against that clean RC1 profile. W6-10C remains inactive until this requalification is reviewed.
