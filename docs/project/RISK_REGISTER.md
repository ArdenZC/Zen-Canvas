...
| R-PLAT-001 | P2 | macOS provider/external/network-volume and broader race behavior is less verified than core native mutation path | V2.1 uses ABI-correct provider identity, bounded materialization/cache, strict destructive namespace proof and target-first `source_cleanup_pending` recovery; bind expanded race/performance and any real-volume claims to exact-head Apple Silicon fixtures, otherwise record **NOT VERIFIED — fixture unavailable** | active |
