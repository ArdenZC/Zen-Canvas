...
The accepted release candidate is `8b573772d842b4996bc1c34161236fa47025cc83` / tree `67cf3da35d7556bb868746a9ae0a56725558a163`. Exact-SHA `CI Full Validation` run `33942690517` completed successfully. `Build Release Installers` run `33943755887` also completed successfully on the same SHA after the qualification gate passed.

Before that final qualification, #189 fixed a real tag-publication blocker: duplicate Node/Rust SBOM generation across both platform matrix lanes would have produced four CycloneDX files while the final publication verifier requires exactly two. The accepted workflow now emits the Node and Rust SBOM pair once, preserves the fail-closed final count, and fresh installer evidence confirms exactly two valid CycloneDX 1.6 SBOMs together with verified Windows/macOS checksum manifests.
