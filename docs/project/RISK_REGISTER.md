...
| R-REL-001 | P2 | Packaged artifacts, unsigned distribution behavior or deferred update capability are misrepresented as a published/reputation-accepted/auto-updating release | Four-state Implemented/Validated/Packaged/Released vocabulary; W5-02 exact-SHA release qualification; W5-03 manual-download/install first-release policy with updater `NOT IMPLEMENTED / DEFERRED`; W5-04 explicitly records native/manual SmartScreen/Gatekeeper/platform acceptance as `UNVERIFIED / DEFERRED`; W5-06 owns the explicit publication-risk decision and must not promote that gap to PASS; `STATUS.md` records no published release/tag | active |
...
W5-03 selected manual download/install for the first public release. W5-04 could not execute the native Windows/macOS GUI acceptance path because the available Computer Use surface exposes browser only (`apps: []`), so SmartScreen/Unknown Publisher, Gatekeeper/quarantine, native install/copy/first-launch and selected accessibility/focus/display observations remain `UNVERIFIED / EXPLICITLY DEFERRED`. This is an accepted residual release risk, not a PASS. W5-06 now owns the explicit decision to hold publication or accept that residual risk and authorize publication. Any future updater must separately own its update-authenticity key, endpoint/manifest, artifact, version, privilege and rollback risks before implementation.
