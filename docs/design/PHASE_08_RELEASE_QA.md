...
- Phase 8.0.1 本轮基线：`ae58a1c62bff1336b00c7746151db61c0a7acaeb`
...
- 100,000 行 SQLite/FTS：search P95 `2.196ms`，total P95 `5.002ms`，阈值 `1000ms`，通过。
...
| `npm test` | 退出码 0；60 files / 386 tests passed |
| `npm run test:performance` | 退出码 0；2 files / 9 tests passed；100,000 rows，search P95 2.196ms，total P95 5.002ms，阈值 1000ms |
...
- 当前 Browser surface 使用 Vite Browser mock，只能证明 DOM、主题、焦点、文案和展示层；不能替代真实 Tauri/Rust 后端运行。
- Windows 原生 125%/150% DPI、200% 文本缩放、系统高对比度、macOS 原生窗口和真实屏幕阅读器仍未在本环境实机验证。
- 旧的 Phase 8 截图和机器证据仍保存在外部目录 `C:\Users\77588\.codex\artifacts\zen-canvas-phase8-final-qa\`；本轮没有把它们复制到仓库。

## 12. Phase 8.0.1 自动化与响应式收口附录

本附录记录本轮小范围 correctness closeout。范围包括 AI runtime 状态同步、英文文案、Spotlight 复数、Settings 容器 scroll-spy、AI 分类预设、共享文件图标、Suggestions/Automation 安全语义和 History 单复数密度修正；不新增业务能力、不进入 Stage 9、不改变 Rust 执行安全链。

### 12.1 状态与规则模型

- AI 设置区分已持久化设置、当前草稿、未保存状态、保存中状态、保存失败回滚和运行时已生效状态；只有 `saveAISettings` 成功后才发布 AppShell runtime state。
- 保存失败恢复上一次已持久化值；旧 load 响应带版本保护，不能覆盖较新的 publish。页面切换/重新挂载、disabled、local/cloud 与 developer mode 由测试覆盖。
- AI 分类预设从真实字段值反推 `Fast / Standard / Detailed / Custom`；Custom 表示手动组合，切换 provider 保留 API key 和无关的 batch/concurrency 草稿。

### 12.2 Settings、Spotlight、Suggestions 与图标一致性

- Settings 使用主内容容器内的 scroll-spy；点击分组、手动滚动、到达底部和 Spotlight 搜索范围锚点共享同一分组语义，页面级 Preferences 标题不重复。
- Spotlight 入口和弹窗均包含 folders；结果、最近文件、File Library、Suggestions 复用 `FileTypeIcon` 的 PDF/document、archive、image、installer、folder 和 unknown 映射。
- Suggestions 只保留一个决定状态徽标；普通 Inspector 文案不暴露内部枚举，并明确 `Suggestion → user decision → Preview → explicit confirmation → execution`。
- Automation 空状态与当前环境说明不再暴露 mock、fixture 或 backend limitation 语言；手动运行继续只使用页面展示的启用 user rules。
- History 批次和恢复摘要使用共享 plural helper，避免 `1 items`。

### 12.3 测试与 Browser QA 证据

- 新增/更新测试文件：`tests/phase801Closeout.test.tsx`、`tests/appArchitecture.test.ts`、`tests/commandModalUi.test.ts`、`tests/settingsViewUi.test.ts`、`tests/searchSpotlight.test.ts`、`tests/organizeV42.test.ts`、`tests/rulesViewUi.test.ts`。
- 当前前端全量结果：60 个 test files、386 个 tests 通过；`npm run test:performance` 通过，100,000 rows，search P95 `2.196ms`、total P95 `5.002ms`。
- 当前 Browser live proof 写入外部目录 `C:\Users\77588\.codex\artifacts\zen-canvas-phase8-0-1\`；本轮记录 Spotlight 0/1/many、Settings active/bottom、主题 Light/Dark/System、AI local/cloud、Suggestions、Automation、History singular 和横向溢出检查。
- 完整尺寸截图与历史机器证据位于 `C:\Users\77588\.codex\artifacts\zen-canvas-phase8-final-qa\`，覆盖 1440×900、1280×800、1180×720、1100×700、1024×700、1000×700、981×680、980×680、900×650 的 Light/Dark；这些均为外部证据，不提交。
- Browser console 最终检查没有 error/warning；Vite debug 日志中的 HMR invalidate 仅属于开发服务器热更新，不作为产品 warning/error。

本附录完成后停止在 Phase 8.0.1，不进入 Stage 9；没有削弱文件移动、重命名、删除、覆盖、Preview、白名单、冲突检查、Safe Trash 或 Restore 边界。
