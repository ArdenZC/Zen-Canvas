...
| **D8**（prehash 多轮语义修订） | 修订 D7 必改 1 的部分结论。**"单例跳过全量"在多轮视角下不成立**：跳过的单例保持未哈希 → 下轮该组重新入选且已含已哈希成员 → 混合组 → 直接全量；总账字节一分未省、推迟一轮、多付采样、组多挂一轮候选。裁决三条：(1) 晋级规则**全有或全无**——全部子组单例才整组跳过（唯一持久节省，动机场景为同 size 不同内容的大文件），任一子组 ≥2 则含单例全组本轮全量（推迟严格劣于现在做）；(2) `bytes_saved_by_prehash` **只计全互异组**，5.1(a) 估算明确为上限口径，T7 夹具须构造全互异组与含重复组并按此断言；(3) 两笔明账记录不加机制——全互异组每轮 8KB×N 重采样租金、此类组永久留在候选集；租金成为实际问题时由独立任务书评估 prehash 落库。**附带更正**：D7 中弃 (b) 的"混合组必须全量"理由是循环论证（(b) 下已哈希成员 prehash 为运行时现算，淘汰路径存在）；真实理由为采样不持久（每轮重付 8KB×N）与组永不落定（与本条同根源）。**T2 开工前本修订必须已落进 `01-dedupe.md`**——已完成 | `01-dedupe.md` 头部 D8 修订记录与 5.1/5.2/T6/T7 | **生效**，修订 D7 |
