...
- Task 02 任务书由人工完整编写，不拆分为 02A/02B 等授权任务；本任务书合并到 `master` 后，Task 02 可执行。
...
| 02 | `TASK_02_IDENTITY_FINGERPRINT_AND_DUPE.md` | watcher rule recovery debt、physical identity、fingerprint cache、prehash/full hash、durable run、hardlink-safe duplicate groups | **任务书已批准；合并到 master 后可执行** |
