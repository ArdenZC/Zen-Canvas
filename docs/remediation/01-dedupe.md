...
> **2026-07-26 D8 修订（T2 前置硬约束）**：多轮语义修正——"prehash 单例跳过全量"在多轮视角下只是推迟而非节省（跳过的单例保持未哈希 → 下轮组变混合 → 直接全量）。晋级规则改为**全有或全无**：全部子组单例才整组跳过（唯一持久节省），任一子组 ≥2 则含单例全组本轮全量。`bytes_saved_by_prehash` 只计全互异组；弃 (b) 的循环论证已更正（真实理由：采样不持久 + 组永不落定）。两笔明账（重采样租金、候选集常驻）已记录。
...
- **全未哈希组是 prehash 唯一有效的场景**（首次运行 / 批量新文件）。按 (size, prehash) 分组后的晋级规则为**全有或全无（D8 修订）**：
  - **所有**子组均为单例（组内内容全互异）→ **全组跳过全量哈希**。这是 prehash 唯一的持久节省：组保持全未哈希，之后每轮只付 8KB×N 的采样租金，永不全量读——恰为动机场景（等码率视频、镜像、备份包：同 size 不同内容的大文件）。
  - 存在**任一**子组 ≥2 → 该子组进入全量，**且单例成员也在本轮一并全量**。多轮账本推演（D8）：若本轮跳过单例，其 `content_hash` 保持为空 → 下一轮该 size 组重新入选，且此时组内已含已哈希成员 → 属混合组 → 单例直接全量。总账是字节数一分未省、只推迟一轮，还多付本轮采样并让组多挂一轮候选——**推迟严格劣于现在做**。
...
(a) 实施前用索引估算收益**上限**（只统计 prehash 实际启用的全未哈希组）：
  ```sql
  ...
  ```
  SQL 无法预知哪些组会全互异（哈希未算），故该值是**仅当全部组实测全互异时才实现的上限**。实际持久节省 = 全互异组的 `Σ(size×n − 8KB×n)`。
(b) run 结果记录 `bytes_saved_by_prehash` 与 `prehash_pruned_files`，**口径均只计全互异组的跳过（D8）**——按全有或全无规则，不存在"被推迟的字节"，任何进入全量的组贡献为 0；该口径与 T7 验收对齐：夹具须同时构造已知的全互异组与含重复组，断言 `bytes_saved_by_prehash` 恰等于构造的全互异组字节（±10% 容差仅覆盖 IO 计量误差），含重复组不得计入。
...
- 弃 (b)（对已哈希成员限量采样）：**（D8 更正）** 早先记录的理由（"混合组必须全量，采样无淘汰能力"）是循环论证——(b) 之下已哈希成员的 prehash 是运行时现算的，若全组采样，与所有成员 prehash 均互异的未哈希成员确实可被淘汰，淘汰路径存在。弃 (b) 的**真实理由**是多轮账本：其一，采样不持久，每轮对已哈希成员重付 8KB×N；其二，被淘汰的成员 `content_hash` 永远为空，组**永不落定**、每轮重新入选（与 D8 第 2.1 节同根源）。复杂度换来的是一条每轮付租金且永不收敛的路径。
- 取 (c)：缓存命中的文件**在任何路径上都不再产生 IO**，论证与设计一致；prehash 的收益集中于全未哈希组，正是它唯一有效的场景（Q1，晋级规则见 D8 全有或全无）。

**两笔明账（D8，记录即可，本轮不加机制）**：
1. **全互异组的重采样租金**：此类组每轮付 8KB×N 采样。动机场景（少数大文件）下可忽略；极端情形（如上万个同 size 分块文件）为每轮数十 MB 读，仍远优于全量。
2. **此类组永久留在候选集**：`candidate_sizes` 每轮都会选中它们，带来固定的查询与采样开销。

若未来观测到租金或候选集膨胀成为实际问题，由独立任务书评估 prehash 落库；本轮不做。
...
│   │           → 成员头尾采样 → (size, prehash) 分组 → 全有或全无（D8）：
│   │             全部子组单例 → 整组跳过（唯一持久节省）；
│   │             任一子组 ≥2 → 全组成员（含单例）本轮全量
│   │           已哈希成员一律不采样（方案 (c)）；混合组未哈希成员直接全量
...
| 6 | 单元测试：prehash 淘汰正确性（同头不同尾/同尾不同头/整读小文件/混合组直接全量）、**全有或全无晋级（D8）**：存在 ≥2 子组时单例本轮一并全量、全互异组整组跳过且成员保持未哈希、`bytes_saved_by_prehash` 仅计全互异组、并发写 CAS、取消 flush、run 状态机 CAS、interrupted 恢复、**keyset 翻页在大量 `wasted_bytes` 并列组下逐页翻遍不漏不重**（必改 3） | 高 | T2/T3 | 低 | 否 | 否 | 全绿且不削弱既有断言 |
| 7 | 集成与性能：10k 文件夹具**同时构造已知的全互异组与含重复组**，断言 `bytes_saved_by_prehash` 恰等于全互异组字节（±10% 仅覆盖 IO 计量误差，含重复组不得计入，D8 口径）；哈希吞吐 ≥ 单线程基线 ×2（4 核 SSD）；**顺序读占优介质场景（必改 2）**：优先真实机械盘实测 `WORKERS=1` 不劣于默认值的回退有效性；CI 无法提供该介质时，**此项验收缺口显式记录进 closeout**，以「环境变量回退 + 已记录风险」代替，不得声称已覆盖 | 中 | T2/T6 | 中（机器与介质差异） | 否 | 否 | 性能夹具入 `test:performance` 或 cargo ignored 基准 |
